    ...
    for (var i = 0; i < gBoard.length; i++) {
        for (var j = 0; j < gBoard[i].length; j++) {
            ...
            if (isEmptyCell(nextCoord)) {
                if (pieceCoord.i === nextCoord.i && pieceCoord.j === nextCoord.j)
                    continue
                if (pieceCoord.i === nextCoord.i) res.push(nextCoord)
                if (pieceCoord.j === nextCoord.j) res.push(nextCoord)
            }
        }
    }
    return res;
...
function getAllPossibleCoordsKnight(pieceCoord) {
    ...
    var diff1 = 1
    for (var i = 0; i < 2; i++) {
        var nextCoord = { i: pieceCoord.i + diff2, j: pieceCoord.j + 1 }
        diff2 = -diff2
        if (isEmptyCell(nextCoord)) { res.push(nextCoord) }
        var nextCoord = { i: pieceCoord.i + diff2, j: pieceCoord.j - 1 }
        if (isEmptyCell(nextCoord)) { res.push(nextCoord) }
    ...
    for (var i = 0; i < 2; i++) {
        diff1 = -diff1
        var nextCoord = { i: pieceCoord.i + diff1, j: pieceCoord.j + 2 }
        if (isEmptyCell(nextCoord)) { res.push(nextCoord) }
        var nextCoord = { i: pieceCoord.i + diff1, j: pieceCoord.j - 2 }
        if (isEmptyCell(nextCoord)) { res.push(nextCoord) }
    }
